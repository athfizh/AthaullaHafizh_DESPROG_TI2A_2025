...
<form method="POST" action="<?php echo $form_action; ?>">
    ...
    <div class="form-group">
        ...
        <input type="text" id="first_name" name="first_name" class="form-input" 
               value="<?php echo $is_edit ? htmlspecialchars($employee['first_name']) : ''; ?>" required placeholder="Masukkan nama depan">
    </div>

    <div class="form-group">
        <label for="last_name" class="form-label">Nama Belakang *</label>
        <input type="text" id="last_name" name="last_name" class="form-input" 
               value="<?php echo $is_edit ? htmlspecialchars($employee['last_name']) : ''; ?>" required placeholder="Masukkan nama belakang">
    </div>

    <div class="form-group">
        <label for="email" class="form-label">Email *</label>
        <input type="email" id="email" name="email" class="form-input" 
               value="<?php echo $is_edit ? htmlspecialchars($employee['email']) : ''; ?>" required placeholder="contoh@company.com">
    </div>
    ...
    <div class="form-group">
        ...
        <input type="text" id="position" name="position" class="form-input" 
               value="<?php echo $is_edit ? htmlspecialchars($employee['position']) : ''; ?>" required placeholder="Masukkan posisi/jabatan">
    </div>

    <div class="form-group">
        <label for="salary" class="form-label">Gaji (Rp) *</label>
        <input type="number" id="salary" name="salary" class="form-input" 
               value="<?php echo $is_edit ? $employee['salary'] : ''; ?>" required min="0" step="100000" placeholder="Contoh: 5000000">
    ...
    <div class="form-group">
        ...
        <input type="date" id="hire_date" name="hire_date" class="form-input" 
               value="<?php echo $is_edit ? $employee['hire_date'] : ''; ?>" required>
    </div>
...
<script>
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        const salary = salaryInput.value.replace(/\D/g, ''); // Hapus non-digit
        ...
        if (parseInt(salary) < 1000000) {
            alert('Gaji minimal Rp 1.000.000!');
            e.preventDefault();
            return false;
        }
        
        // Pastikan nilai salary yang dikirim adalah angka bersih
