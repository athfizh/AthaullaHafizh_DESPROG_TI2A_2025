...
<?php if ($stats->rowCount() > 0): ?>
    ...
    $stats->execute(); // Jalankan ulang query jika perlu
                ...
                <tr>
                    ...
                    <td>
                        <strong style="color: #27ae60;">
                            Rp <?php echo number_format($dept['total_salary_budget'], 0, ',', '.'); ?>
                        </strong>
                    </td>
                </tr>
            <?php endforeach; ?>
    ...
    <div style="margin-top: 3rem;">
        ...
        <div style="background: white; padding: 1.5rem; border-radius: 8px; margin: 1rem 0; border-left: 4px solid #667eea;">
            ...
            foreach ($all_stats as $dept): 
                $width_percent = ($max_avg_salary > 0) ? ($dept['avg_salary'] / $max_avg_salary * 100) : 0;
            ?>
                <div style="margin: 0.5rem 0;">
                    ...
                    <div style="background: #f0f0f0; border-radius: 4px; height: 20px;">
                        <div style="background: #667eea; height: 100%; border-radius: 4px; width: <?php echo $width_percent; ?>%;"></div>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
        ...
        <div style="background: white; padding: 1.5rem; border-radius: 8px; margin: 1rem 0; border-left: 4px solid #27ae60;">
            ...
            <?php 
            $max_total_employees = max(array_column($all_stats, 'total_employees'));
            foreach ($all_stats as $dept): 
                $width_percent = ($max_total_employees > 0) ? ($dept['total_employees'] / $max_total_employees * 100) : 0;
            ?>
                <div style="margin: 0.5rem 0;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.25rem;">
                        <span><?php echo htmlspecialchars($dept['department']); ?></span>
                    ...
                    <div style="background: #f0f0f0; border-radius: 4px; height: 20px;">
                        <div style="background: #27ae60; height: 100%; border-radius: 4px; width: <?php echo $width_percent; ?>%;"></div>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
